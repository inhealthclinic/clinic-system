    ...
    <div style={{ minHeight: '100vh', background: '#F5F5F5', fontFamily: "'Inter', sans-serif" }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ width: 28, height: 28, background: '#0B63C2', borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M9 3H5a2 2 0 0 0-2 2v4"/><path d="M9 3h6"/><path d="M15 3h4a2 2 0 0 1 2 2v4"/><path d="M3 9v6"/><path d="M21 9v6"/><path d="M3 15v4a2 2 0 0 0 2 2h4"/><path d="M21 15v4a2 2 0 0 1-2 2h-4"/><path d="M9 21h6"/>
              <line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/>
            </svg>
          </div>
          <span style={{ fontSize: 15, fontWeight: 600, color: '#141414' }}>Лаборатория</span>
        </div>
        <button onClick={() => { setForm(emptyForm); setPatientQuery(''); setShowForm(true) }} style={{ background: '#0B63C2', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 18px', fontSize: 14, fontWeight: 500, cursor: 'pointer' }}>
          + Новый анализ
        </button>
      </div>
      ...
      <div style={{ padding: '24px', maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
          {[['all', 'Все'], ['pending', 'Ожидает'], ['in_progress', 'В работе'], ['ready', 'Готов']].map(([v, l]) => (
          ...
          <input placeholder="Поиск по пациенту или анализу..." value={search}
            ...
            <form onSubmit={saveTest}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                <div>
                  ...
                  <input style={inputStyle} value={form.ordered_by} onChange={e => setForm(f => ({ ...f, ordered_by: e.target.value }))} placeholder="Врач" />
                </div>
                <div>
      ...
      {showResult && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 16 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: '32px', width: '100%', maxWidth: 480 }}>
            ...
            <button onClick={() => setShowResult(null)} style={{ width: '100%', padding: '10px 0', border: '1px solid #D1D5DB', borderRadius: 8, background: '#fff', fontSize: 14, cursor: 'pointer', color: '#374151' }}>Закрыть</button>
          </div>
        </div>
      )}
    </div>
