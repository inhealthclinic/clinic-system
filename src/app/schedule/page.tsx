...
export default function SchedulePage() {
  ...
  async function saveAppointment(e: React.FormEvent) {
    ...
    setSaving(false)
    setShowForm(false)
    // if the new appointment is in current week range — reload
    loadWeek()
  }
    ...
    <div style={{ minHeight: '100vh', background: '#F5F5F5', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #E8EDF5', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 60 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 15, fontWeight: 600, color: '#141414' }}>Расписание</span>
        </div>
        <button onClick={openForm} style={{ background: '#0B63C2', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 18px', fontSize: 14, fontWeight: 500, cursor: 'pointer' }}>
          + Новый приём
        </button>
      </div>
          ...
          <div style={{ background: '#fff', borderRadius: 16, padding: '32px', width: '100%', maxWidth: 500, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <form onSubmit={saveAppointment}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <input style={inputStyle} required value={form.doctor_name} onChange={e => setForm(f => ({ ...f, doctor_name: e.target.value }))} placeholder="ФИО врача" />
              </div>

              {/* Service */}
              <div style={{ marginBottom: 16 }}>
                <label style={labelStyle}>Услуга</label>
                <input style={inputStyle} value={form.service} onChange={e => setForm(f => ({ ...f, service: e.target.value }))} placeholder="Консультация, УЗИ, анализы..." />
              </div>
