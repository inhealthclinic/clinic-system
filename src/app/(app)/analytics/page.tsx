...
type Stats = {
  totalPatients: number; newPatientsMonth: number
  appointmentsToday: number; appointmentsMonth: number
  appointmentsCompleted: number; appointmentsCancelled: number
  revenueToday: number; revenueMonth: number; revenueWeek: number
  totalLeads: number; leadsConverted: number; leadsLost: number
  labTestsReady: number; labTestsPending: number
...
function Bar({ value, max, color }: { value: number; max: number; color: string }) {
  return (
    <div style={{ height: 6, background: '#F3F4F6', borderRadius: 3, overflow: 'hidden' }}>
      <div style={{ height: '100%', width: `${max ? Math.round((value / max) * 100) : 0}%`, background: color, borderRadius: 3, transition: 'width 0.4s' }} />
    </div>
  )
}

const STATUS_COLORS: Record<string, { bg: string; text: string }> = {
  scheduled: { bg: '#EEF4FF', text: '#0B63C2' }, arrived: { bg: '#ECFDF5', text: '#059669' },
...
export default function AnalyticsPage() {
  ...
  async function loadStats() {
    ...
    try {
      ...
      setStats({
        totalPatients: totalPatients || 0, newPatientsMonth: newPatientsMonth || 0,
        appointmentsToday: (apptToday || []).length,
        appointmentsMonth: (apptMonth || []).length,
        appointmentsCompleted: (apptMonth || []).filter(a => a.status === 'completed').length,
        appointmentsCancelled: (apptMonth || []).filter(a => a.status === 'cancelled').length,
        revenueToday: paid.filter(p => p.date === today).reduce((s, p) => s + Number(p.amount), 0),
        revenueMonth: paid.reduce((s, p) => s + Number(p.amount), 0),
        revenueWeek: paid.filter(p => p.date >= weekStart).reduce((s, p) => s + Number(p.amount), 0),
        totalLeads: (leads || []).length,
        leadsConverted: (leads || []).filter(l => l.status === 'converted').length,
        leadsLost: (leads || []).filter(l => l.status === 'lost').length,
        labTestsReady: (labTests || []).filter(t => t.status === 'ready').length,
        labTestsPending: (labTests || []).filter(t => t.status !== 'ready').length,
        topSources: Object.entries(sourceCounts).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([source, count]) => ({ source, count })),
        topServices: Object.entries(serviceCounts).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([service, count]) => ({ service, count })),
  ...
  return (
    <div style={{ padding: '24px', maxWidth: 960, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 600, color: '#141414', margin: '0 0 2px' }}>Аналитика</h1>
          <p style={{ fontSize: 13, color: '#6B7280', margin: 0 }}>Данные обновляются в реальном времени</p>
        </div>
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
            <div style={{ background: '#fff', border: '1px solid #E8EDF5', borderRadius: 12, padding: '20px 24px' }}>
              ...
              <p style={{ fontSize: 12, color: '#6B7280', margin: '12px 0 0', paddingTop: 12, borderTop: '1px solid #F3F4F6' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
            <div style={{ background: '#fff', border: '1px solid #E8EDF5', borderRadius: 12, padding: '20px 24px' }}>
              ...
              {stats.topSources.length === 0 ? <p style={{ fontSize: 13, color: '#9CA3AF' }}>Нет данных</p>
            ...
            <div style={{ background: '#fff', border: '1px solid #E8EDF5', borderRadius: 12, padding: '20px 24px' }}>
              ...
              {stats.topServices.length === 0 ? <p style={{ fontSize: 13, color: '#9CA3AF' }}>Нет данных</p>
